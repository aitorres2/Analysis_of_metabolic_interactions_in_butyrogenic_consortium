...
#-----------------------------------------------------------------------------------------------
ggplot(data = M62_data, aes(x = Time_h)) +
...
M62_data_long <- M62_data %>%
  gather(key = "Replica", value = "ln_OD", ln_R1, ln_R2, ln_R3)

# Usar ggplot2 para crear el gráfico
ggplot(data = M62_data_long, aes(x = `Time_h`, y = ln_OD, color = Replica)) +
  geom_point() +
  geom_line() +
  theme_minimal() +
...
start_exponential_phase <- 7.00002778
end_exponential_phase <- 15.0004722
...
growth_rate_R1 <- get_growth_rate(M62_data, start_exponential_phase, end_exponential_phase, "ln_R1")
growth_rate_R2 <- get_growth_rate(M62_data, start_exponential_phase, end_exponential_phase, "ln_R2")
growth_rate_R3 <- get_growth_rate(M62_data, start_exponential_phase, end_exponential_phase, "ln_R3")
...
M62_data$ln_OD_mean <- rowMeans(M62_data[, c("ln_R1", "ln_R2", "ln_R3")])

# Luego, realiza la regresión lineal en la fase exponencial
exponential_data <- filter(M62_data, Time_h >= start_exponential_phase & Time_h <= end_exponential_phase)
...
OD_final_R1 <- M62_data$R1[95]
OD_final_R2 <- M62_data$R2[95]
OD_final_R3 <- M62_data$R3[95]
...
OD_values <- c(0.32555, 0.33215, 0.29645)
