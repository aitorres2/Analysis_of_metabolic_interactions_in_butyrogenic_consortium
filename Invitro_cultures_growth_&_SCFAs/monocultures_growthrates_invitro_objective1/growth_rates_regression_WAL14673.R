...
#-----------------------------------------------------------------------------------------------
ggplot(data = WAL14673_data, aes(x = Time_h)) +
...
WAL14673_data_long <- WAL14673_data %>%
  gather(key = "Replica", value = "ln_OD", ln_R1, ln_R2, ln_R3)

# Usar ggplot2 para crear el gráfico
ggplot(data = WAL14673_data_long, aes(x = `Time_h`, y = ln_OD, color = Replica)) +
  geom_point() +
  geom_line() +
  theme_minimal() +
...
growth_rate_R1 <- get_growth_rate(WAL14673_data, start_exponential_phase, end_exponential_phase, "ln_R1")
growth_rate_R2 <- get_growth_rate(WAL14673_data, start_exponential_phase, end_exponential_phase, "ln_R2")
growth_rate_R3 <- get_growth_rate(WAL14673_data, start_exponential_phase, end_exponential_phase, "ln_R3")
...
WAL14673_data$ln_OD_mean <- rowMeans(WAL14673_data[, c("ln_R1", "ln_R2", "ln_R3")])

# Luego, realiza la regresión lineal en la fase exponencial
exponential_data <- filter(WAL14673_data, Time_h >= start_exponential_phase & Time_h <= end_exponential_phase)
...
OD_final_R1 <- WAL14673_data$R1[95]
OD_final_R2 <- WAL14673_data$R2[95]
OD_final_R3 <- WAL14673_data$R3[95]
...
OD_values <- c(0.455, 0.49839999, 0.5538)
