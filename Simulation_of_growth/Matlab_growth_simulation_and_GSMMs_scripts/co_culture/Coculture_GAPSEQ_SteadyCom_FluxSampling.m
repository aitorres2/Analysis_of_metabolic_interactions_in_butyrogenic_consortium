...
models = {Clostridium_innocuum_HFG2, Bifidobacterium_animalis_lactis_PT33};
Names = {'Clostridium_innocuum_HFG2', 'Bifidobacterium_animalis_lactis_PT33'};

%% Establish biomass a objective function per model
objectiveAbbr_Clostridium_innocuum_HFG2 = checkObjective(Clostridium_innocuum_HFG2);
...
medioPUC = {
    'EX_cpd11602_e0';... % 'EX_inulin(e)'(EX_cpd11602);...
    'EX_cpd02298_e0';... FOS
    ...
    'EX_cpd00218_e0';... %not registered in VMH. 
    'EX_cpd00215_e0';...'EX_pydx(e)'
    'EX_cpd00443_e0';... %not registered in VMH. 
    ...
    'EX_cpd00305_e0';... 'EX_thm(e)',... 
    'EX_cpd00541_e0';... 'EX_lipoate(e)',... 
    'EX_cpd00028_e0';... 'EX_pheme(e)',... 
...
for i = 1:length(models)
    for j = 1:length(media)
        ...
        'VariableNames',{'rxn','v'});
         ...
         writetable(Bifidobacterium_animalis_lactis_PT33_mono_table,'Bifidobacterium_animalis_lactis_PT33 _monocultivo_modelseed.xlsx');
...
Clostridium_innocuum_HFG2_table = table(EcCom.infoCom.EXsp(find(EcCom.indCom.EXsp(:,1) ~= 0),1), ... 
    sol.full(EcCom.indCom.EXsp(EcCom.indCom.EXsp(:,1) ~= 0,1)), ... 
    'VariableNames',{'rxn','v'})
...
Bifidobacterium_animalis_lactis_PT33_table(~Bifidobacterium_animalis_lactis_PT33_table.v,:) = []; % https://www.mathworks.com/matlabcentral/answers/407401-delete-zero-rows-from-a-table
Clostridium_innocuum_HFG2_table(~Clostridium_innocuum_HFG2_table.v,:) = [];
com_table(~com_table.v,:) = [];

Bifidobacterium_animalis_lactis_PT33_table.rxn = regexprep(Bifidobacterium_animalis_lactis_PT33_table.rxn, '[u]tr', '(e)'); % me reemplza los _e por TT
Clostridium_innocuum_HFG2_table.rxn = regexprep(Clostridium_innocuum_HFG2_table.rxn, '[u]tr', '(e)'); % me reemplza los _e por TT
com_table.rxn = regexprep(com_table.rxn, '[u]tr', '(e)'); % me reemplza los _e por TT

writetable(Bifidobacterium_animalis_lactis_PT33_table,'Bifidobacterium_animalis_lactis_PT33_consorcio_con_Clostridium_innocuum_HFG2_modelseed.xlsx');
writetable(Clostridium_innocuum_HFG2_table,'Clostridium_innocuum_HFG2_consorcio_con_Bifidobacterium_animalis_lactis_PT33_modelseed.xlsx');
writetable(com_table,'EX_comunidad_Bifidobacterium_animalis_lactis_PT33_Clostridium_innocuum_HFG2_modelseed.xlsx');
...
options.rxnNameList = strcat('X_', Names); % set the variables to look for relative abundances in the steadycom FVA
...
saveas(p(j, 1),'EX_BM_comunidad_Bifidobacterium_animalis_lactis_PT33_Clostridium_innocuum_HFG2_modelseed.png');
hold off

%% FVA optimizing for metabolite of interest (butyrate)
options.optBMpercent = 100;
options.rxnNameList = EcCom.rxns(find(contains(EcCom.rxns, 'IEX_cpd00211'))); % butyrate reaction
options.optGRpercent = [0:1:100];
options.GRtol = 1e-6;
[fvaComMin_B, fvaComMax_B, minFD_B, maxFD_B, GRvector_B, result_B, LP_B] = SteadyComFVA(EcCom, options);

%% %% Envelope plot for butyrate by community growth rate
grComV = result.GRmax * options.optGRpercent / 100; 
lgLabel = {strrep(Names{1}, '_', '\_')};  % name the butyrate-producing bacterium
f = figure;
x_lower = 10; 
x_upper = 0; 
hold on
...
tl(1) = title('\underline{SteadyComFVA butyrate}', 'Interpreter', 'latex');
lgLabel2 = {strrep(Names{1}, '_', '\_')}; % % name the butyrate-producing bacterium
lg = legend(lgLabel2);
yl(1) = ylabel('Butyrate flux (mmol gDW^{-1} h^{-1})');
xl(1) = xlabel('Community growth rate (h^{-1})');

% Save the combined figure
saveas(p(j, 1),'EX_combinado_Bifidobacterium_animalis_lactis_PT33_Clostridium_innocuum_HFG2_modelseed.png');
hold off
...
EcCom = changeRxnBounds(EcCom,'Clostridium_innocuum_HFG2bio1_biomass',result.vBM(1),'b');
EcCom = changeRxnBounds(EcCom,'Bifidobacterium_animalis_lactis_PT33IEX_biomass[c]tr',result.vBM(1),'b');
EcCom = changeRxnBounds(EcCom,'Bifidobacterium_animalis_lactis_PT33biomass345',result.vBM(2),'b');
EcCom = changeRxnBounds(EcCom,'Bifidobacterium_animalis_lactis_PT33IEX_biomass[c]tr',result.vBM(2),'b');
...
writematrix(samples,'samples_Bifidobacterium_animalis_lactis_PT33_Clostridium_innocuum_HFG2_200000_modelseed_ids.csv') 

%% Export sampling reaction names
EcCom_rxns = EcCom.rxns
writecell(EcCom_rxns,'samples_EcComRXNS_Bifidobacterium_animalis_lactis_PT33_Clostridium_innocuum_HFG2_modelseed_ids.csv') 
